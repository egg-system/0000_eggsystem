...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			$this->strings = array(
				'page_title'                      => __( 'Install Required Plugins', 'Grazioso' ),
				'menu_title'                      => __( 'Install Plugins', 'Grazioso' ),
				'installing'                      => __( 'Installing Plugin: %s', 'Grazioso' ),
				'oops'                            => __( 'Something went wrong.', 'Grazioso' ),
				'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'このテーマを使用するには、次のプラグインのインストール、有効化が必須となります： %1$s.' ),
				'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'このテーマを活用するために次のプラグインのインストールを推奨します： %1$s.' ),
				...
				'install_link' 					  => _n_noop( 'プラグインをインストールする', 'プラグインをインストールする' ),
				'update_link' 					  => _n_noop( 'プラグインを更新する', 'プラグインを更新する' ),
				'activate_link' 				  => _n_noop( 'インストールしたプラグインを有効化する', 'インストールしたプラグインを有効化する' ),
				'return'                          => __( '推奨プラグインをインストールする', 'Grazioso' ),
				'plugin_activated'                => __( 'プラグインを有効化しました。', 'Grazioso' ),
				'complete'                        => __( '全てのプラグインを有効化しました。 %1$s', 'Grazioso' ),
			);
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) || ( isset( $_GET[sanitize_key( 'tgmpa-update' )] ) && 'update-plugin' == $_GET[sanitize_key( 'tgmpa-update' )] ) ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( 'Return to Required Plugins Installer', 'Grazioso' ) . '</a></p>';
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'Grazioso' ) . '">' . __( 'Return to the Dashboard', 'Grazioso' ) . '</a>' ) . '</p>';
			...
			elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
						$msg = sprintf( __( 'The following plugin was activated successfully: %s.', 'Grazioso' ), '<strong>' . $plugin['name'] . '</strong>' );
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						$rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'Grazioso' ), $imploded, $count ) . '</p>'; // All messages now stored
					}

					/** Setup variables to determine if action links are needed */

					$show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'Grazioso' ) . '</a>' : '';
					$show_activate_link = $activate_link ? '<a href="' . admin_url( 'plugins.php' ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'Grazioso' ) . '</a>'  : '';
					$show_update_link  = $update_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'Grazioso' ) . '</a>' : '';
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( 'これらの必須プラグイン等に関する通知を非表示にする', 'Grazioso' ) . '</a>',
						)
					);
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', 'Grazioso' );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( 'Private Repository', 'Grazioso' );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( 'Pre-Packaged', 'Grazioso' );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress Repository', 'Grazioso' );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', 'Grazioso' ) : __( 'Recommended', 'Grazioso' );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'Grazioso' ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'Grazioso' ) );
				elseif ( version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<' ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Updated', 'Grazioso' ) );
				else
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Active', 'Grazioso' ) );
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'Grazioso' ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'Grazioso' ),
				'source' => __( 'Source', 'Grazioso' ),
				'type'   => __( 'Type', 'Grazioso' ),
				'status' => __( 'Status', 'Grazioso' )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', 'Grazioso' ),
				'tgmpa-bulk-activate' => __( 'Activate', 'Grazioso' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'Grazioso' ), $imploded );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', 'Grazioso' );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'Grazioso' );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'Grazioso' );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'Grazioso' );
				$this->strings['process_failed']      = __( 'Plugin install failed.', 'Grazioso' );
				$this->strings['process_success']     = __( 'Plugin installed successfully.', 'Grazioso' );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'Grazioso' );
				$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'Grazioso' );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'Grazioso' );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'Grazioso' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'Grazioso' ) . '</span><span class="hidden">' . __( 'Hide Details', 'Grazioso' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'Grazioso' );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'Grazioso' );
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'Grazioso' );
					$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'Grazioso' );
					$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'Grazioso' );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'Grazioso' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'Grazioso' ) . '</span><span class="hidden">' . __( 'Hide Details', 'Grazioso' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'Grazioso' );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'Grazioso' );
				}
			...
			public function bulk_footer() {
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'Grazioso' ) . '">' . __( 'Return to the Dashboard', 'Grazioso' ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
