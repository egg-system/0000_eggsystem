 ...
 */
 ...
 class My_Theme_Options {
	...
	public function display_page() {
		...
		<h1 class="options_title">' . __( 'テーマオプション', 'lang' ) . '</h1>
	...
	// メディアアップローダのロード
			jQuery(\'input.select-image\').click(function(){
				url_input =jQuery(this).parent(\'td\').find(\'input.imgurl-text\');
				...
				tb_show(null, \'media-upload.php?post_id=0&type=image&TB_iframe=true&tab=library\');
				return false;
			})
...
if ($.browser.mozilla) 
	...
	public function display_setting( $args = array() ) {
		...
		switch ( $type ) {
			...
			case 'textarea':
			echo '<textarea class="' . $field_class . '" id="' . $id . '" name="mytheme_options[' . $id . ']" placeholder="' . $std . '" rows="5" cols="30">' . format_for_editor( $options[$id] ) . '</textarea>';
			...
			case 'image':
			echo '<input class="imgurl-text' . $field_class . '" type="text" id="' . $id . '" name="mytheme_options[' . $id . ']" placeholder="' . $std . '" value="' . esc_attr( $options[$id] ) . '" /><input type="button" class="button select-image" value="画像選択" rel="">';
	...
	public function get_option() {
		...
		$this->settings['archive_background_image'] = array(
			'section' => 'appearance',
			'title' => __( 'タイトル背景画像', 'lang' ),
			'desc' => __( 'アーカイブページタイトル部分の背景画像を選択してください （空の場合は背景色）', 'lang' ),
		...
		$this->settings['api_fb_page_id'] = array(
			'title'   => __( 'FACEBOOKページのいいね数を取得したいPAGE ID', 'lang' ),
			'desc'    => __( '', 'lang' ),
			'std'     => '',
			'type'    => 'text',
			'section' => 'api',
		...
		$this->settings['api_fb_access_token'] = array(
			'title'   => __( 'FACEBOOK ACCESS TOKEN', 'lang' ),
			'desc'    => __( '', 'lang' ),
			'std'     => '',
			'type'    => 'text',
			'section' => 'api',
